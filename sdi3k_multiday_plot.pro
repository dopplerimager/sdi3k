...
for j=0,n_elements(skylis)-1 do begin
    ...
    if j eq 0 then nz = mm.nzones
    if mm.nzones ne nz then goto, skip_this_day
    if n_elements(spekfits) gt 1 and n_elements(windpars) gt 1 and n_elements(spekfits) eq n_elements(windpars) then begin
...
;---Setup Plotting:
    ...
    scale = {time_range: [0., 1.], yrange: [150., 350.], auto_scale: 0}
    if strpos(choice.name, 'WIND') ge 0 then scale = {time_range: [0., 1.], yrange: [-300., 300.], auto_scale: 0}
    if strpos(choice.name, 'VERTICAL') ge 0 then scale = {time_range: [0., 1.], yrange: [-100., 100.], auto_scale: 0}
    if choice.index ge 11 then scale = {time_range: [0., 1.], yrange: [-1., 1.], auto_scale: 0}
