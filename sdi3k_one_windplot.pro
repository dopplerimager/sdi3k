...
;hourangle=0
 ;hourangle = 22*!pi/180.
     ...
     if keyword_set(no_project) then begin
        cx =  geo.radius*windfit(rcd).zeniths(zidx)*sin(!dtor*(windfit(rcd).azimuths(zidx) + azoff - mm.oval_angle))/mm.sky_fov_deg
        cy =  geo.radius*windfit(rcd).zeniths(zidx)*cos(!dtor*(windfit(rcd).azimuths(zidx) + azoff - mm.oval_angle))/mm.sky_fov_deg
     endif
        mer = -mer
        cy  = -cy
        cx  = -cx
     ...
     if mm.latitude lt 0. then begin
       ; zon = -zon
       ; mer = -mer
     end
