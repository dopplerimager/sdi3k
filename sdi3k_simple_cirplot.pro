...
pro sdi3k_simple_cirplot, filename, plotarr, canvas_size=canvas_size, center=center, temp_range=temp_range, $
                          rgb_file=rgb_file, thick=thk, records=rcd, windscale=windscale, minute_step=minstep, white_background=bg_white, $
                          window_index=window_index, no_legend=no_legend
...
;          where: xx and yy are netCDF filenames
    ...
    sdi3k_read_netcdf_data, filename, metadata=mm, winds=winds, spekfits=spekfits, zonemap=zonemap, zone_centers=zone_centers; , images=images
...
;               spekfits.intensity(k) = tbrite
    ...
    if not(keyword_set(thk)) then thk = 2
...
;---Determine the wavelength:
    ...
    if ~keyword_set(window_index) then while !d.window ge 0 do wdelete, !d.window
...
;---Build the time information arrays:
    ...
    if keyword_set(rcd) then begin
       rcd = [rcd(0) > 0, rcd(1) < (n_elements(tlist) - 1)]
    endif else begin
    ...
    if keyword_set(minstep)   then tstep = minstep else begin
...
;---Setup dial plotting:
    ...
    if doing_green  then scale = {yrange: 100., auto_scale: 0, minute_step: tstep, magnetic_midnight: mm.magnetic_midnight, rbscale: tprscale, gscale: britescale, pscale: [0, 9e9]}
    ...
    if keyword_set(windscale) then scale.yrange      = windscale

    if not(keyword_set(canvas_size)) then canvas_size=[750, 690]
    xsize = canvas_size(0)
    ysize = canvas_size(1)
    pmarg = 3
    if not(keyword_set(window_index)) then begin
       while !d.window ge 0 do wdelete, !d.window
       window, xsize=xsize, ysize=ysize
    endif else wset, window_index
    ...
    cirplot_settings = {scale: scale, black_bgnd: 1, geometry: geo, records: rcd}
    if keyword_set(bg_white) then cirplot_settings.black_bgnd = 0
    sdi3k_dial_mapper, tlist, tcen, mm, winds, cirplot_settings, culz, spekfits, zonemap, center=center, thick=thk, no_legend=no_legend; , images=images
    plotarr = tvrd(/true)
