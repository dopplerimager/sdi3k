    ...
    ccd_size = 0.008192
    efl      = 0.3
    if strpos(strupcase(mapfile), 'HAARP') ge 0 or strpos(strupcase(mapfile), 'TOOLIK') ge 0 then begin
       efl = 0.395
       ccd_size = 0.013312
    endif
    ...
    yy    = lindgen(nn,nn)/nn
    cc    = where(base eq min(median(base, 5)))
    cc = cc(0)
    xcen  = xx(cc)
    ycen  = yy(cc)
...
;    ycen  = nn/2
    ...
    goods = where(rr gt 100 and rr lt 240)
...
;    xx = dialog_pickfile(path="D:\users\SDI3000\Data", /multi)
    xx = dialog_pickfile(path="D:\users\conde\main\Poker_SDI\Downrange_FPIs\Toolik\First_Light_Plots", /multi)
