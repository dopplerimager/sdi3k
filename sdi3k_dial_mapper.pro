pro sdi3k_dial_mapper, tlist, tcen, mm, windfit, cirplot_settings, culz, spekfits, zone_map, images=images, $
                       center=center, thick=thk, no_legend=no_legend
...
;---Setup background and pen colors:
    if cirplot_settings.black_bgnd then begin
       ...
       arrow_color = culz.white
    endif else begin
       bgnd      = culz.white
       pen_color = culz.black
       arrow_color=culz.black
    endelse
    erase, color=bgnd
...
;    mbox  = max([xsize, ysize])/2.
    ...
    xyouts, xcen, ycen - 0.51*prad, 'Magnetic Midnight', alignment=0.5, $
...
;           img = culz.imgmin + bytscl(img, min=iimg.scale(0), max=iimg.scale(0)+smax, top=culz.imgmax - culz.imgmin - 1)
        ...
        sdi3k_one_windplot, windfit, tcen, rec, geo, mm, thick=thk, color=arrow_color, index_color=-1, /no_project
    endif
    endfor
...
;    yhi = convert_coord(50, 70, /device, /to_normal)
    ...
    if n_elements(images) le 2 or ~(use_images) then begin
       ...
       barspot = [0.92, 0.80, 0.95, 0.95]
       if mm.latitude lt 0 then barspot = [0.05, 0.80, 0.08, 0.95]
       mccolbar, barspot, 0, 255, rgblimz(0,0), rgblimz(1,0), $
