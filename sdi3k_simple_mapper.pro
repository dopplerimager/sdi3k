...
pro sdi3k_simple_mapper, filename, plotarr, canvas_size=canvas_size, what_plot=what_plot, $
                         time_smoothing=tsm, space_smoothing=ssm, yrange=yrange, scale=inscale

    if not(keyword_set(what_plot)) then what_plot = 'WINDMAPS'
    what_plot = strupcase(what_plot)

    sdi3k_read_netcdf_data, filename, metadata=mm, winds=winds, spekfits=spekfits, images=images, $
                            zonemap=zonemap, zone_centers=zone_centers
...
;---Apply any zero-velocity offset correction maps that have been selected:
    if doing_green and getenv('SDI_GREEN_ZERO_VELOCITY_FILE') ne '' then begin
       ...
       for j=0,n_elements(spekfits) - 1 do begin
           spekfits(j).velocity = spekfits(j).velocity - wind_flat_field.wind_offset
       endfor
    endif
...
;       if doing_red    then tprscale = [medtemp - 200. > 0, medtemp + 200.]
       if doing_red    then tprscale = [800., 1000.]
;       if doing_green  then tprscale = [medtemp - 250. > 0, medtemp + 250.]
       if doing_green  then tprscale = [275, 575]
...
;---Set the bitmap size for skymaps:
    ...
    scale = {yrange: [-100., 100.], auto_scale: 0}
...
;---Setup the wind mapping:
    ...
    if doing_red    then scale = {yrange: 200., auto_scale: 0, rbscale: tprscale, gscale: britescale, pscale: [0, 9e9]}
    ...
    if keyword_set(inscale) then scale=inscale

    cadence  = fix((jhi-jlo)/30) > 1
    geo   = {xsize:  xsize, ysize: ysize}
    pp  = 'Map'
