...
filter     = ['*_2012_*_843*.*', '*_2012_*6328_NZ0043.*']
obj_edt, filter
...
if flatfield ne 'None' then begin
   restore, flatfield
   for j=0, n_elements(spekfits) -1 do spekfits(j).velocity = spekfits(j).velocity - wind_flat_field.wind_offset
endif


fact = 3
if mm.columns lt 512 then fact = 4
...
plot_options = {plot_images: 1, plot_temperature: 1, plot_LOS_winds: 0, plot_insprofs: 1, plot_wind_vectors:1, $
                image_scale: [0.04, 0.98], histogram_auto_scale: 1, image_smoothing: 12., temperature_range: [300,1000], wind_scale: scale, window_size: [mm.columns, mm.rows]} ;##########
...
;           iimg   = rebin(iimg, mm.rows, mm.columns)
           iimg   = congrid(iimg, plot_options.window_size(0), plot_options.window_size(1))
...
;restore, 'F:\users\SDI3000\Data\Poker\Wind_flat_field_PKR_5577A_created_2010_06_19_by_discard.sav'   ;###3
 ;          indata  = [[wind_offset*metadata(0).channels_to_velocity], [spekfits(rec).intensity], [spekfits(rec).temperature]] ; ##########
   ...
   window, xsize=plot_options.window_size(1), ysize=plot_options.window_size(1), title="GI/UAF All-Sky Fabry-Perot Composite Display"
   xpix    = plot_options.window_size(0)
   ypix    = plot_options.window_size(0)
...
;goto, skip_spex  ; ##############
    for zidx=0,mm.nzones-1 do begin
        ...
        skycul=culz.white
        if abs(skewarr(rec,zidx) - medskew) gt (2*skewdev > 0.16) or spekfits(rec).chi_squared(zidx) gt 30 then skycul = culz.cyan
;        if spekfits(rec).chi_squared(zidx) gt 20 then skycul = culz.cyan
       ...
       !p.position =  0
       ;xyouts, zone_centers(j, 0), zone_centers(j, 1), strcompress(string(j, format='(i3)'), /remove_all), color=culz.white, charsize=1, align=0.5, charthick=2, /normal
    endfor
skip_spex:  ;##############
