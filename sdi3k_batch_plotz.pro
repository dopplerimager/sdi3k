...
;---Determine the wavelength:
    ...
    minsnr = 150.
    if doing_oh then minsnr = 80.

    goods = where(median(spekfits.signal2noise, dim=1) gt minsnr, ng)
    if ng lt 3 then return
    ...
    if keyword_set(skip_existing) then skip_existing = 1
...
;---Setup briteness and temperature scaling:
    ...
    medtemp = 100*round(medtemp/100)
    tprscale= [medtemp - 250. > 0, medtemp + 250.]
...
;    if doing_red    then tprscale = [500., 1100.]
    ...
    if doing_OH     then tprscale = [medtemp - 350. > 0, medtemp + 350.]
    f107 = 70.
...
;---Median Temperature Time Series:
    ...
    if doing_OH     then tsplot_settings.msis.msis_height = 87.
...
;---Scatter plot temperature against signal2noise:
    scale = {auto_scale: 0, xrange: [100., 5e4], yrange: [0., 1500.]}
    ...
    if doing_OH     then scale = {auto_scale: 0, xrange: [100., 5e3], yrange: [0., 800.]}
...
;---Map LOS winds:
    ...
    if doing_OH     then scale = {yrange: [-100., 100.], auto_scale: 0}
    geo   = {xsize:  xsize, ysize: ysize}
...
;---Setup the wind mapping:
    scale = {yrange: 150., auto_scale: 0, rbscale: tprscale, gscale: britescale, pscale: [0, 9e9]}
    ...
    if doing_OH     then scale = {yrange: 120., auto_scale: 0, rbscale: tprscale, gscale: britescale, pscale: [0, 9e9]}
...
;---Setup dial plotting:
    if n_elements(winds) lt 3 then return
    scale = {yrange: 150., auto_scale: 0, minute_step: 40., magnetic_midnight: mm.magnetic_midnight, rbscale: tprscale, gscale: britescale, pscale: [0, 9e9]}
    ...
    if doing_OH     then scale = {yrange: 120., auto_scale: 0, minute_step: 40., magnetic_midnight: mm.magnetic_midnight, rbscale: tprscale, gscale: britescale, pscale: [0, 9e9]}
    if doing_green  then load_pal, culz, idl=[8,  0]
    if doing_sodium then load_pal, culz, idl=[14, 0]
    if doing_red    then load_pal, culz, idl=[3,  0]
    ysize = 1500
